    ...
    const iframeStyles = `
        * {
        ...
        .n8n-fullscreen-button {
            position: absolute;
            right: 47%; /* Adjusted to place it to the left of the refresh button (48px + 28px) */
            top: 47%;
            transform: translateY(-50%);
            background: none;
            border: none;
        ...
        console.warn('Speech recognition is not supported in this browser.');
            ...
            if (fullscreenButton) {
                fullscreenButton.addEventListener('click', () => {
                    // console.log('fullscreenButton button clicked from inside iframe');
...
window.addEventListener('message', (event) => {
    if (event.data.type === 'fullscreenButton') {
        ...
        if (!isFullscreen) {
            ...
            iframe.style.borderRadius = '0'; // Remove border radius in fullscreen
    ...
    // Resize functionality for iframe
    let isResizing = false;
    // let startX, startY, startWidth, startHeight, startLeft, startTop;
    ...
    window.addEventListener('message', (event) => {
        if (event.data.type === 'closeChat') {
            ...
            const dx = startX - event.data.clientX;
            const dy = startY - event.data.clientY;

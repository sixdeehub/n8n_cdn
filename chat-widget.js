...
(function() {
    ...
    const iframeContent = `
        ...
        <script>
            ...
            var TrulienceAvatarID = "";
            var TrulienceAvatarToken = "";

            const trulienceparams = "";
            const trulienceURL = "";
            ...
            function setupToggleListener() {
                if (!isVideoEnabled) {
                    ...
                    if (toggleInput) {
                        toggleInput.closest('.n8n-toggle-switch')?.remove();
                    }
            ...
            function enableSecureInput() {
                const textarea = document.getElementById('n8n-chat-textarea');
                if (document.getElementById('n8n-secure-input')) return; // already active

                // Create password input overlaying the textarea
                const secureInput = document.createElement('input');
                secureInput.type = 'password';
                secureInput.id = 'n8n-secure-input';
                secureInput.placeholder = 'Enter OTP...';
                secureInput.autocomplete = 'one-time-code';

                // Copy textarea styles so it looks identical
                const styles = window.getComputedStyle(textarea);
                secureInput.style.cssText = textarea.style.cssText;
                secureInput.style.width = styles.width;
                secureInput.style.height = styles.height;
                secureInput.style.font = styles.font;
                secureInput.style.padding = styles.padding;
                secureInput.style.border = styles.border;
                secureInput.style.borderRadius = styles.borderRadius;
                secureInput.style.background = styles.background;
                secureInput.style.color = styles.color;
                secureInput.style.outline = styles.outline;
                secureInput.style.boxSizing = styles.boxSizing;
                secureInput.style.resize = 'none';
                secureInput.style.display = 'block';
                secureInput.style.width = '100%';

                // Hide textarea, insert secure input in same position
                textarea.style.display = 'none';
                textarea.parentNode.insertBefore(secureInput, textarea);

                secureInput.focus();
                secureInput.addEventListener('input', updateButtonVisibility);
                // Add this inside enableSecureInput(), after secureInput.focus()
            ...
            function disableSecureInput() {
                const secureInput = document.getElementById('n8n-secure-input');
                const textarea = document.getElementById('n8n-chat-textarea');
                if (secureInput) {
                    secureInput.remove();
                }
            ...
            function updateButtonVisibility() {
                const secureInput = document.getElementById('n8n-secure-input');
                const activeInput = secureInput || textarea;
                
                const hasInput = activeInput.value.trim().length > 0 || currentFiles.length > 0;
                sendButton.style.display = hasInput ? 'flex' : 'none';
                micButton.style.display = hasInput && !isRecording ? 'none' : 'flex';
            }
            ...
            const sessionLastResponse = new Map();
            ...
            async function sendMessage(message, files = []) {
                ...
                if (message) {
                    const lastBotResponse = sessionLastResponse.get(currentSessionId);
                    const shouldMask = lastBotResponse?._requiresMasking === true;

                    const messageTextDiv = document.createElement('div');
                    messageTextDiv.textContent = shouldMask
                        ? '*'.repeat(message.length)
                        : message;
                    messageTextDiv.setAttribute('data-otp-masked', shouldMask ? 'true' : 'false');
                    messageBubble.appendChild(messageTextDiv);
                }
                ...
                try {
                                ...
                                for (const file of files) {
                                    try {
                                        ...
                                        if (
                                            ...
                                            socket.send(
                                                JSON.stringify(combinedMessage)
                                            );
                    ...
                    function handleOrderedResponse(responseMsgId, rawMessage, table_content, dropdown_content, hasMore = false, isOtpResponse = false) {
                        // console.log('Handling ordered response for msg_id:', responseMsgId, 'hasMore:', hasMore);
                        const sequence = messageSequences.get(responseMsgId);
                        
                        // Remove any unordered loader
                        const unorderedLoader = messagesContainer.querySelector(
                            '.n8n-loading-indicator[data-loading-for="unordered"]'
                        );

                        if (unorderedLoader) {
                            unorderedLoader.remove();
                        }


                        if (!sequence) {
                            console.warn('No sequence found for:', responseMsgId);
                            return handleUnorderedResponse(
                                rawMessage,
                                table_content,
                                dropdown_content,
                                hasMore,
                        ...
                        // console.log('isOtpResponse in handleOrderedResponse:', isOtpResponse);

                        // Apply your existing message formatting
                        if_video_is_off(botMessageDiv, rawMessage, table_content, dropdown_content, isOtpResponse);
                        ...
                        // const expectMoreResponses = data.has_more === true;                        

                        if (hasMore && !sequence.loadingDiv) {
                    ...
                    function handleUnorderedResponse(rawMessage,table_content,dropdown_content,hasMore = false,isOtpResponse = false) {

                        // Remove all existing loaders
                        document.querySelectorAll('.n8n-loading-indicator')
                            .forEach(loader => loader.remove());

                        // Also clear references from sequences
                        messageSequences.forEach(sequence => {
                            sequence.loadingDiv = null;
                        });
                        // Remove existing unordered loader
                        const existingLoader = messagesContainer.querySelector(
                            '.n8n-loading-indicator[data-loading-for="unordered"]'
                        );
                        ...
                        messagesContainer.scrollTop = messagesContainer.scrollHeight;
                        if (hasMore) {
                            const loadingDiv = createLoadingIndicator('unordered');
                            messagesContainer.appendChild(loadingDiv);
                        }
                    }
                    ...
                    function if_video_is_off(botMessageDiv,rawMessage,table_content, dropdown_content, isOtpResponse) {
                        let messageBodyHTML = '';
                            let fulltableHTML = '';
                            // console.log('isOtpResponse in if_video_is_off:', isOtpResponse);
                            if (!isOtpResponse) {
                                if (table_content && Array.isArray(table_content.columns) && Array.isArray(table_content.rows)) {
                    ...
                    function handleResponse(data) {
                        ...
                        if (data != null) {
                            ...
                            // saving the last response for the current session
                            const isOtpResponse = !!otpItem;
                            // console.log('Is OTP response:', isOtpResponse);
                            data._requiresMasking = isOtpResponse;
                            sessionLastResponse.set(currentSessionId, data);
                            // console.log('Session last response updated for sessionId:', currentSessionId, 'Data:', data);


                            // ✅ Toggle secure input mode on textarea
                            const textarea = document.getElementById('n8n-chat-textarea');
                            if (isOtpResponse) {
                                textarea.setAttribute('type', 'password'); // won't work on textarea
                                // So we swap to a password input instead
                                enableSecureInput();
                            } else {
                                disableSecureInput();
                            }
                            ...
                            const hasMore = data.has_more === true; // Adjust based on your server response
                            
                            if (responseMsgId) {
                                // console.log('Handling ordered response with msg_id:', responseMsgId);
                                handleOrderedResponse(responseMsgId, rawMessage, table_content, dropdown_content, hasMore,isOtpResponse);
                            } else {
                                // console.warn('No msg_id found in response, falling back to unordered handling.');
                                // Fallback to old behavior if no msg_id
                                handleUnorderedResponse(rawMessage, table_content, dropdown_content,hasMore,isOtpResponse);
                            }
            ...
            sendButton.addEventListener('click', () => {
                ...
                if (message || currentFiles.length > 0) {
                    ...
                    activeInput.value = '';
                    if (!secureInput) autoResizeTextarea(textarea);
                    currentFiles = [];
                    updateButtonVisibility();
                    renderFilePreviews();

                    // Disable secure mode after OTP is sent
                    disableSecureInput();
                }
            });
